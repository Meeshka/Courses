...
def test_multiplication():
    ...
    print(f"Multiplied dimensions {mult_tensor.ndim}")

def test_broadcasting():

    #the tensors can be multiplied if all their sizes fit, or if one or more of sub sizes is 1
    ...
    tensor_rand = torch.floor(torch.rand(4, 3, 1)*10+1).to(torch.int32)
...
def test_backward_grad():
    tensor_base = Variable(torch.ones(2, 2), requires_grad=True)
    print(f"Base tensor: {tensor_base}")
    tensor_step2 = tensor_base * 2
    print(f"Next step tensor: {tensor_step2}")
    tensor_step3 = torch.pow(tensor_step2, 2)
    tensor_step4 = tensor_step3.mean()
    print(f"Yet next tensor {tensor_step3}",tensor_step4)
    tensor_step4.backward()
    print(f"Backward base tensor: {tensor_base.grad}")
